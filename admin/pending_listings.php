...
if (role('manager')) {
    $listings = $db->query('SELECT * FROM `listing` WHERE completed = 0 AND admin = ?;', $_SESSION['admin'])->fetchAll();
} else {
    $listings = $db->query('SELECT * FROM `listing` WHERE completed = 0;')->fetchAll();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $p = $_POST;
    if (role('manager')) {
        $listings = $db->query('SELECT * FROM `listing` WHERE completed = 0 AND `admin` = ? AND `foreclosure_date` BETWEEN ? AND ?;', $_SESSION['admin'], $p['start_date'], $p['end_date'])->fetchAll();
    } else {
        $listings = $db->query('SELECT * FROM `listing` WHERE completed = 0 AND `foreclosure_date` BETWEEN ? AND ?;', $p['start_date'], $p['end_date'])->fetchAll();
    }
}
...
if (isset($_GET['listing_id']) && $_GET['listing_id'] !== '') {
    ...
    if (isset($_GET['action'])) {
        ...
        if ($_GET['action'] == 'delete') {
            ...
            if ($delete_listing) {
                redirect('Listing Deleted Successfully!', ADMIN . '/pending_listings.php');
            }
        }
        ...
        if ($_GET['action'] == 'notcomplete') {
            ...
            if ($hide_listing) {
                redirect('Listing Mark as Not Completed Successfully!', ADMIN . '/pending_listings.php');
            }
        }
        ...
        if ($_GET['action'] == 'complete') {
            ...
            if ($hide_listing) {
                ...
                redirect('Listing Mark as Completed Successfully!', ADMIN . '/complete_listings.php');
            }
        }
    }
}
...
<head>
    ...
    <title>Pending Listings - PromisedLand</title>
</head>
                                ...
                                <table id="datatablesSimple">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($listings as $item) {
                                            ...
                                            $actions .= '<a href="' . ADMIN . '/pending_listings.php?listing_id=' . $item['id'] . '&action=delete" class="btn btn-danger" title="Delete Listing" onclick="return confirm(\'Are you sure to delete this listing?\');"><i class="fa fa-trash-alt"></i></a>';
                                            ...
                                            echo '<tr>';
                                            echo '<td>' . $item['id'] . '</td>';
                                            echo '<td class="text-center"><input type="checkbox" name="listing_checked[]" value="' . $item['id'] . '"></div></td>';
